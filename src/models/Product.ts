...
ProductSchema.pre("save", function (this: IProduct, next) {
  ...
  const retail = this.prices.retail || 0;
  const discount = this.prices.discount || 0;
  this.prices.effectivePrice = Math.max(retail - discount, 0);
  // next();
});
...
ProductSchema.index({ "prices.retail": 1 }); // sorting by price
ProductSchema.index({ "prices.discount": 1 }); // for discount filters
ProductSchema.index({ "prices.effectivePrice": 1 }); // for effective price filters
ProductSchema.index({ isFeatured: 1, isActive: 1 }); // for home sections
ProductSchema.index({ tags: 1 }); // tag filters
ProductSchema.index({ totalViews: -1 }); // popular products sorting
ProductSchema.index({ score: -1 }); // top rated products sorting
